...
- Edu. background in Finance / Business IT
...
- More precisely: ["a sequence of characters that specifies a search pattern"](https://en.wikipedia.org/wiki/Regular_expression)
...
```{r, echo = TRUE}
review_words_by_movie <- reviews %>% 
    .[, .(movie_title, short_review)] %>%
    unnest_tokens(output = word, input = short_review)

review_words_by_movie[, .N, by = "word"] %>%
    .[order(-N)] %>%
    head(10)
```
...
```{r, echo = TRUE}
sentiment_scores <- get_sentiments(lexicon = "afinn") %>%
    data.table()
