...
```{r}
pROC::auc(response = test[["is_spam"]], predictor = baseline_predictions)
```
...
```{r}
pROC::auc(response = test[["is_spam"]], predictor = logit_predictions)
```

```{r}
calculateROC(actual = test[["is_spam"]], predictions = logit_predictions) %>%
    plotROC(model_name = "full logit model")
```
