...
rf_model <- ranger::ranger(
    is_spam ~ .,
    data = train[, -c("message_id", "sms_text", "150p")],
    importance = "impurity"
)

rf_predictions <- predict(rf_model, data = test, type = "response")[["predictions"]]

rf_auc <- calculateAUC(actual = test[["is_spam"]], predictions = rf_predictions)
...
calculateROC(
    ...
    plotROC(
        model_name = "Random Forest using all features, non-tuned",
        auc = rf_auc
    )
...
data.table(
    ...
    ggplot(aes(x = reorder(variable, importance), y = importance)) +
        ...
        coord_flip() +
        labs(
            title = "Variable importances (based on impurity) in the Random Forest model",
            x = "variable"
        ) +
        theme_minimal()
```
