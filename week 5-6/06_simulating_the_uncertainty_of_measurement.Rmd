...
### 7. Calculate the 'sales amount per contact' for the control and treatment group with uncertainty both with bootstrapping and based on the variance in sales amount! Compare the results!
```{r}
# based on variance
# SA_var <- dt[,
# 	.(sales_amount_per_contact = mean(sales_amount),
# 	  SD_sales_amount_per_contact = sd(sales_amount),
# 	  num_contact = .N),
# 	by = group
# ] %>%
# 	.[, .(
# 		group, sales_amount_per_contact,
# 		SE = SD_sales_amount_per_contact / sqrt(num_contact)
# 	)]

# ggplot(SA_var, aes(x = group, y = sales_amount_per_contact)) +
# 	geom_col() +
# 	geom_errorbar(aes(ymin = sales_amount_per_contact - SE * 1.96, ymax = sales_amount_per_contact + SE * 1.96))
```
...
# set.seed(1234)
# SA_bs_uncertainty <- map_df(1:1000, ~{
# 	dt[sample(.N, .N, replace = TRUE)] %>%
# 		.[,
# 			.(sales_amount_per_contact = mean(sales_amount),
# 			  bootstrap_sample_id = .x),
...
# }) %>%
# 	.[,
# 		.(CI_lower = quantile(sales_amount_per_contact, 0.025),
# 		  CI_higher = quantile(sales_amount_per_contact, 0.975)),
# 		by = group
# 	]

# merge(SA_aggr, SA_bs_uncertainty, by = "group") %>%
# 	ggplot(aes(x = group, y = sales_amount_per_contact)) +
# 		geom_col() +
# 		geom_errorbar(aes(ymin = CI_lower, ymax = CI_higher))
```